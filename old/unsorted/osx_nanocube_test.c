...
test_old()
{
	...
	PlatformMemory data_memory = platform.allocate_memory(Gigabytes(1), 12, Terabytes(2));
	PlatformMemory insert_memory = platform.allocate_memory(Megabytes(32), 0, 0);
...
test_complete_tree()
{
	...
	PlatformMemory data_memory = platform.allocate_memory(Gigabytes(6), 12, Terabytes(2));

	PlatformMemory insert_memory = platform.allocate_memory(Megabytes(32), 0, 0);

	al_Allocator* allocator = al_Allocator_new(data_memory.memblock);

	al_Cache* nanocube_cache = al_Allocator_create_cache(allocator, "NC", sizeof(nx_NanocubeIndex));

	nx_NanocubeIndex* nanocube_index = (nx_NanocubeIndex*) al_Cache_alloc(nanocube_cache);
	...
	for (u32 i=0;i<n;++i) {
		...
		nx_NanocubeIndex_insert (nanocube_index, &address[0], &payload_unit, 0, insert_memory.memblock);
