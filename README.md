...
To compile the nanocubes toolkit, run the following commands on your linux/mac system.  You can replace `3.1` with other valid release numbers, e.g. 3.0.1, 3.0, 2.1.3, 2.1, 2.0, but in those cases please follow the instructions in the README.md file for those releases as they will differ slightly.

```
wget https://github.com/laurolins/nanocube/archive/3.1.zip
unzip 3.1.zip
cd nanocube-3.1
...
After these commands you should have directory `nanocube-3.1/bin` with the nanocubes toolkit inside. To make these tools more easily accessible in your account, add the `nanocube-3.1/bin` directory to your PATH environment variable.
