...
engine <- nanocube::nanocube.load("taxi=/Users/llins/projects/compressed_nanocube/data/taxi_june.nanocube")
...
tx_query <- function(engine, lat, lon, tillevel, tileres, tilexrange, tileyrange) 
{
...
data            <- tx_query(engine, lat, lon, tilelevel, tileres, tilexrange, tileyrange)
selection       <- data$records$count > 1
data$records    <- data$records[selection,]
values          <- data$records$fare/data$records$count

v <- ggplot(data$records, aes(ximg, yimg, z=values, fill=values))
v + geom_raster(aes(fill=values))
