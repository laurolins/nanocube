...
LinuxApplicationCode;

internal LinuxApplicationCode
linux_load_application_code(char *application_dll_path, Print *print)
{
	LinuxApplicationCode result;
	...
	result.application_process_request = (ApplicationProcessRequest*)
		dlsym(result.application_code_dll,
...
main(int num_args, char** args)
{
	...
	pt_Memory profile_memory = osx_linux_allocate_memory(profile_memory_size, 3, 0);
	...
	linux_init_platform(&app_state.platform);
	...
	LinuxApplicationCode app_code = linux_load_application_code(dll_path.full_path, &print);

#if 0
	osx_linux_ThreadInfo thread_info_queue[3];
	pt_WorkQueue work_queue;
	if (!osx_linux_init_work_queue(&work_queue, thread_info_queue, ArrayCount(thread_info_queue))) {
		Print_clear(&print);
	...
	{
		app_state.platform.write_to_file(&platform_stdout, print.begin, print.end);
		Print_cstr(&print, "\n[Problem] Could not dynamically load library: ");
...
#ifdef PROFILE
	osx_linux_free_memory(&profile_memory);
#endif
