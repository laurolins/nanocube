...
#define LOG_COMPUTE_TREE_DECOMPOSITION_VERBOSITY 2
...
namespace polycover {
	...
	bool ChainBoundary::Iterator::next() {
		...
		if (current_y == -1) {
			...
			while (current_y < static_cast<int>(boundary.rows.size()) && boundary.rows[current_y] == UNDEFINED)
				++current_y;
			if (current_y >= static_cast<int>(boundary.rows.size()))
				return false;
			else
				return true;
		}
	...
	void ChainBoundary::insert(int x, int y) {
		...
		//
		// 2019-07-21T14:58:10 this exception is being triggered...
		// go to the pure C polycover library ASAP and avoid C and C++
		// interactions
		//
		if (x < 0 || y < 0) {
			return;
	...
	void CellMap::markCellsThatIntersectPolygon(LLPolygon &llpoly) {
		...
		while (true) {
			...
			std::cerr << "processing vertex at position: " << u->point << std::endl;
#endif
...
#ifdef LOG_MARK_CELLS_THAT_INTERSECT_POLYGON
				std::cerr << "....vertex at " << u_type.u->point << " on cell " << cell_addr << " is " << c << std::endl;
#endif
	...
	/*!
	 * Auxiliar class to count iteration_tag in a CoverTreeEngine.
	 */
	...
	/*!
	 * Generata a random simple grid polygon
	 */
	Polygon random_simple_grid_polygon(int width, int height, int num_sides, uint32_t seed)
	{
		...
		while (static_cast<int>(point_set.size()) < num_sides) {
	...
	void compute_cover_raster_segment(RealPoint p1, RealPoint p2, std::vector<GridPoint>& result ) {
		...
		if (horizontal && vertical) {
			...
			auto iterate = [](const RealVec &p, const RealVec &delta) -> RealVec {
				...
				auto alpha_x = (next_x - p.x)/delta.x; // these are all positive use of delta
				auto alpha_y = (next_y - p.y)/delta.y; // these are all positive use of delta


				// std::cerr << "alpha_x: " << alpha_x << std::endl;
				// std::cerr << "alpha_y: " << alpha_y << std::endl;
			...
			auto p    = p1;
			auto prev = p;
			result.push_back(cast(p,toInt));
			while (true) {

				if (length(p-p1) > len) {
					auto p_end = cast(p2,toInt);
					if (p_end != result.back()) {
						result.push_back(p_end);
						// std::cerr << " (end)  insert " << p_end << std::endl;
					}
					break;
				}

				auto p_med_grid = cast((p + prev)/2.0,toInt);
				if (p_med_grid != result.back()) {
					result.push_back(p_med_grid);
					// std::cerr << " (med)  insert " << p_med_grid << std::endl;
				}

				auto p_grid = cast(p,toInt);
				if (p_grid != result.back()) {
					result.push_back(p_grid);
					// std::cerr << " (next) insert " << p_grid << std::endl;
				}
	...
	labeled_tree::LabeledTree TileCoverEngine::computeBorder(const std::vector<Polygon> &polygons) {

		// result structure shared by all polygon rasterization
		labeled_tree::CoverTreeEngine engine;

		double grid_size = 1ULL << max_level;
		auto transform = [&grid_size](const RealPoint& p) {
			auto aux = ((p - RealPoint{-1.0,-1.0})/RealPoint{2.0,2.0});
		...
		for (auto &p: polygons) {
			...
			auto prev = transform(p.getVertex(static_cast<int>(n-1)));
			for (auto i=0;i<n;++i) {
				auto curr = transform(p.getVertex(i));
				compute_cover_raster_segment(prev, curr, grid_cells);
				for (auto c: grid_cells) {
					engine.goTo(labeled_tree::Path(maps::Tile{c.x, c.y, max_level}));
					engine.consolidate();
				}
				prev = curr;
			}
		...
		// go through each edge adding cells that we hit

		// return engine.
		engine.goTo(labeled_tree::Path());

		std::unique_ptr<labeled_tree::Node> root;
		engine.swap(root);

		labeled_tree::LabeledTree result { std::move(root) };
		result.optimize();

		return result;
	}

	labeled_tree::LabeledTree TileCoverEngine::computeCover(const std::vector<Polygon> &mercator_polygons, bool makeCW) {
		...
		for (auto p: mercator_polygons) {
			...
			if (makeCW) { // guarantee polygons vertices are in Clock-Wise order
				p.makeItCW();
			}
		...
		// There is still a problem here. If max polygon side fits into level "x",
		// it still might need one extra level
		...
		ProcessPolyType process = [&](const std::vector<Polygon> &polygons, const RealBox& bbox, int recursion_level) {
			...
			// we assume here that current_depth > 0
			// otherwise the program should already have
			// solved
			if (current_depth == 0) {
				if (t0 != t1) {
					throw std::runtime_error("No grid to decompose polygon");
				}
			...
			// since t1 was computed as a right boundary
			// a degenerate polygon can have one or both
			// of its grid coordinates smaller than the
			// corresponding grid coordinate of t0.
			...
			// compensate for the fact that we have already used one level
			// of branching to get to current depth
			...
			if (recursion_level + 1 < static_cast<int>(levels.size())) {
		...
		std::stringstream ss;
		if (!result.empty()) {
			labeled_tree::json(ss, *result.root());
			std::cerr << ss.str() << std::endl;
			std::cerr << *result.root() << std::endl;
		}
#endif
