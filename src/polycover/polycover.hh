	...
	template <typename Reporter>
		Grid raster_polygon(const Polygon& polygon, Reporter *reporter) {
			...
			if (bbox.pmin.x < 0.0 || bbox.pmin.y < 0.0)
				throw std::runtime_error("raster_polygon problem");
...
#define xLOG_RASTER_POLYGON_2
	...
	template <typename Reporter>
		Grid raster_polygon2(const std::vector<Polygon> &polygons, Reporter *reporter) {
			...
			if (bbox.pmin.x < 0.0 || bbox.pmin.y < 0.0)
				throw std::runtime_error("polycover::raster_polygon2 problem (out of bounds!)");
	...
	public:
		...
		labeled_tree::LabeledTree computeCover(const std::vector<Polygon> &polygons, bool makeCW=true);
		labeled_tree::LabeledTree computeBorder(const std::vector<Polygon> &polygons);
	public:
