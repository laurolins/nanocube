...
struct BasePoint {
    ...
    BasePoint(BaseLength<U,T> x, BaseLength<U,T> y);
...
public:
    ...
    BaseRectangle() = default;
    BaseRectangle(BasePoint<U,T> min, BasePoint<U,T> max);

    inline auto width() const  -> BaseLength<U,T>;
    inline auto height() const -> BaseLength<U,T>;

    inline auto center() const -> BasePoint<U,T>;

    inline auto corner(Corner c) const -> BasePoint<U,T>;
...
template <typename U, typename T> auto operator+(const BaseLength<U,T> &a, const BaseLength<U,T> &b) -> BaseLength<U,T>;
template <typename U, typename T> auto operator-(const BaseLength<U,T> &a, const BaseLength<U,T> &b) -> BaseLength<U,T>;

template <typename U, typename T> auto operator*(const BaseLength<U,T> &a, T v)           -> BaseLength<U,T>;
template <typename U, typename T> auto operator*(T v, const BaseLength<U,T> &a)           -> BaseLength<U,T>;

template <typename U, typename T> auto operator/(const BaseLength<U,T> &a, T v)           -> BaseLength<U,T>;
template <typename U, typename T> auto operator-(const BaseLength<U,T> &a)                     -> BaseLength<U,T>;

//------------------------------------------------------------------------------
// Point Operations
//------------------------------------------------------------------------------

template <typename U, typename T> auto operator+(const BasePoint<U,T> &a, const BasePoint<U,T> &b) -> BasePoint<U,T>;
template <typename U, typename T> auto operator-(const BasePoint<U,T> &a, const BasePoint<U,T> &b) -> BasePoint<U,T>;

template <typename U, typename T> auto operator-(const BasePoint<U,T> &a)                    -> BasePoint<U,T>;
template <typename U, typename T> auto operator*(const BasePoint<U,T> &a, T s)          -> BasePoint<U,T>;

template <typename U, typename T> auto operator*(T s, const BasePoint<U,T> &a)          -> BasePoint<U,T>;
template <typename U, typename T> auto operator/(const BasePoint<U,T> &a, T s)          -> BasePoint<U,T>;
...
inline auto operator "" _lat(long double x)        -> Point<units::degrees>;
inline auto operator "" _lon(long double y)        -> Point<units::degrees>;
inline auto operator "" _lat(unsigned long long x) -> Point<units::degrees>;
inline auto operator "" _lon(unsigned long long y) -> Point<units::degrees>;
inline auto operator "" _px(unsigned long long v)  -> IntegralLength<units::screen>;
inline auto operator "" _px(long double v)         -> Length<units::screen>;
...
auto units_cast(const BaseLength<U,T> &p) -> BaseLength<V,T>;

template <typename V, typename U, typename T>
auto units_cast(const BasePoint<U,T> &p) -> BasePoint<V,T>;

template <typename V, typename U, typename T>
auto units_cast(const BaseRectangle<U,T> &r) -> BaseRectangle<V,T>;
...
template<typename U, typename T>
geometry::BaseLength<U,T>::BaseLength():
    quantity(0)
{}

template<typename U, typename T>
geometry::BaseLength<U,T>::BaseLength(T quantity):
    quantity(quantity)
{}

template <typename U, typename T>
auto geometry::operator+(const BaseLength<U,T> &a, const BaseLength<U,T> &b) -> BaseLength<U,T> {
...
template <typename U, typename T>
auto geometry::operator*(const BaseLength<U,T> &a, T v) -> BaseLength<U,T> {
    return BaseLength<U,T>(a.quantity*v);
}

template <typename U, typename T>
auto geometry::operator*(T v, const BaseLength<U,T> &a) -> BaseLength<U,T> {
    return BaseLength<U,T>(a.quantity*v);
}
...
template <typename U, typename T>
auto geometry::operator-(const BaseLength<U,T> &a) -> BaseLength<U,T> {
...
template <typename U, typename T>
geometry::BasePoint<U,T>::BasePoint(BaseLength<U,T>  x, BaseLength<U,T>  y):
    x(x), y(y)
{}
...
template <typename U, typename T>
auto geometry::operator+(const BasePoint<U,T> &a, const BasePoint<U,T> &b) -> BasePoint<U,T> {
...
template <typename U, typename T>
auto geometry::operator*(const BasePoint<U,T> &a, T s) -> BasePoint<U,T> {
    return BasePoint<U,T>(a.x.quantity * s, a.y.quantity * s);
}

template <typename U, typename T>
auto geometry::operator*(T s, const BasePoint<U,T> &a) -> BasePoint<U,T> {
    return BasePoint<U,T>(a.x.quantity * s, a.y.quantity * s);
}
...
template <typename U, typename T>
auto geometry::operator-(const BasePoint<U,T> &a) -> BasePoint<U,T> {
...
inline auto geometry::operator "" _px(long double v) -> Length<units::screen> {
    return Length<units::screen>(v);
}

inline auto geometry::operator "" _px(unsigned long long v) -> IntegralLength<units::screen> {
    return IntegralLength<units::screen>((int)v);
}

inline auto geometry::operator "" _lat(long double x) -> Point<units::degrees> {
    return Point<units::degrees>(x,0);
}

inline auto geometry::operator "" _lon(long double y) -> Point<units::degrees> {
    return Point<units::degrees>(0,y);
}

inline auto geometry::operator "" _lat(unsigned long long x) -> Point<units::degrees> {
    return Point<units::degrees>(static_cast<double>(x),0);
}

inline auto geometry::operator "" _lon(unsigned long long y) -> Point<units::degrees> {
    return Point<units::degrees>(0, static_cast<double>(y));
}
...
template <typename U, typename T>
geometry::BaseRectangle<U,T>::BaseRectangle(BasePoint<U,T> min, BasePoint<U,T> max):
    min(min), max(max)
{}
...
template <typename U, typename T>
auto geometry::BaseRectangle<U,T>::width() const -> BaseLength<U,T> {
    return max.x - min.x;
}

template <typename U, typename T>
auto geometry::BaseRectangle<U,T>::height() const -> BaseLength<U,T> {
    return max.y - min.y;
}

template <typename U, typename T>
auto geometry::BaseRectangle<U,T>::center() const -> BasePoint<U,T> {
    return (max + min) / (T) 2;
}

template <typename U, typename T>
auto geometry::BaseRectangle<U,T>::corner(Corner c) const -> BasePoint<U,T> {
...
template <typename V, typename U, typename T>
auto geometry::units_cast(const BaseLength<U,T> &p) -> BaseLength<V,T> {
    return BaseLength<V,T>{ p.quantity };
}

template <typename V, typename U, typename T>
auto geometry::units_cast(const BasePoint<U,T> &p) -> BasePoint<V,T> {
    return BasePoint<V,T>{ p.x.quantity, p.y.quantity };
}

template <typename V, typename U, typename T>
auto geometry::units_cast(const BaseRectangle<U,T> &r) -> BaseRectangle<V,T> {
