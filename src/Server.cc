...
Request::Request(mg_connection *conn, const std::vector<std::string> &params,
    ...
    cache_max_age_in_seconds(1800), allowed_origin("*"), logger(logger)
{}
...
void Request::respondJson(std::string msg_content, int return_code)
{
    const std::string sep = "\r\n";

    std::stringstream ss;
    ss << "HTTP/1.1 " << return_code << " OK" << sep
       << "Content-Type: application/json"    << sep
       << "Access-Control-Allow-Origin: "     << allowed_origin << sep
       << "Cache-Control: private, max-age="  << cache_max_age_in_seconds << sep
       << "Content-Length: %d"                << sep << sep
       << "%s";

    response_size = (int) ss.str().size() + (int) msg_content.size();
    // check how a binary stream would work here
    mg_printf(conn, ss.str().c_str(), (int) msg_content.size(), msg_content.c_str());
}

void Request::respondText(std::string msg_content, int return_code)
{
    const std::string sep = "\r\n";
...
void Request::respondOctetStream(const void *ptr, int size, int return_code)
{
    const std::string sep = "\r\n";
...
{
    logger.log("Registering handler: %s", name.c_str());
...
{ // blocks current thread
    ...
    if (event == MG_NEW_REQUEST) {
        ...
        Request request(conn, tokens, logger);

        if (tokens.size() == 0) {
            logger.log("Request URI: %s", uri.c_str());
            std::stringstream ss;
            ss << "bad URL: " << uri;
            request.respondText(ss.str(), 404);
...
void Server::start(int mongoose_threads) // blocks current thread
{
    ...
    if (!ctx) {
        logger.log(LOG_ERR, "Couldn't create mongoose context... exiting!");
    ...
    logger.log("Server on port %d", port);
    ...
    if (is_timing) {
        if (timing_of.is_open()) {
            ...
            if (!timing_of.is_open()) {
                ...
                logger.log(LOG_WARNING, "Could not record requests and times");
                is_timing = false;
                return false;
            }
        }
