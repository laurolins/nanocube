...
//    void setAddress(Address &addr);
    ...
    void add(AddressType &address, Data data, uint64_t weight);
...
template <typename TypeList, typename Data>
void STree<TypeList,Data>::add(AddressType &address, Data data, uint64_t weight)
{
    static std::vector<void*> updated_content(256);
    updated_content.clear();
    UpdateProcedure<
            STree<TypeList,Data>,
            STree<TypeList,Data>::BeginIteratorType> update(address, root, data, updated_content, weight);
}
...
struct Helper {
                            ...
                            uint64_t                         weight)
    {
        ...
        UpdateProcedure<STreeType, NextIteratorType> update(address, content, data, updated_contents, weight);
...
struct Helper<true>
{
                            ...
                            uint64_t                         weight)
    {
        ...
        if (content.add(data,weight) > 0 && STreeType::add_flag == 0) {
    ...
    updated_contents(updated_contents)
{
    ...
    current_structure.trailProperPath(current_dimension_address, stack);
    ...
    while (!stack.empty())
    {
        ...
        else
        {
            ...
            std::clog << fl("",3*index)
                ...
                update::Helper<LAST_DIMENSION>::template exec<NextIteratorType, STreeType, ContentType, CurrentAddressType>(address, content, data, updated_contents, weight);
            }
        }
