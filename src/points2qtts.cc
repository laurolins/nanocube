...
typedef TimeSeries<EventTime, EventCount> TS;
typedef timeseries::Entry<EventTime, EventCount> TSEntry;

const BitSize LevelBits = 25;
typedef QuadTree<LevelBits, TS> QuadTreeTS;
...
{
    void visit(QNode *node, QAddr addr)
    {
        TS *ts = node->getContent();
...
void dumpTypeSizes(std::ostream &os)
{
    ...
    os  << sep
        ...
        << fr(str(sizeof(quadtree::QuadTree<25,TS>))) << sep << std::endl;

    os  << sep
        << fl("TS")              << sep // number of add calls
        << fr(str(sizeof(TS)))   << sep << std::endl;

    os  << sep
        << fl("TS::Entry")              << sep // number of add calls
        << fr(str(sizeof(TSEntry)))   << sep << std::endl;
...
//            break;
    ...
    { // run a count session
        ...
        hitS.visitSubnodes(QAddr(), -1, statsVisitor);
